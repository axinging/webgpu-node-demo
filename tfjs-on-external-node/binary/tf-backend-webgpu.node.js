 ...
 * =============================================================================
 */
var compileProgram = function (device, program, shader, parallelCompilation) {
    var module = device.createShaderModule({ code: shader, label: program.constructor.name });
...
function makeShader(inputInfo, outputInfo, program) {
    var prefixSnippets = [];
    ...
    if (program.isFromPixels) {
        prefixSnippets.push("\n        struct Uniform {\n          size            : i32,\n          numChannels     : i32,\n          outShapeStrides : vec2<i32>,\n        };\n\n        @group(0) @binding(0) var<storage, read_write> result: array<".concat(dataTypeToGPUType(outputInfo.dtype, program.outputComponent), ">;\n        @group(0) @binding(2) var<uniform> uniforms: Uniform;\n      "));
    ...
    var outputInfoType = getCoordsDataType(outputInfo.shape.length);
    uniformDeclaration += "outShape : ".concat(outputInfoType, ", ");
    ...
    var coordsSnippet = getOutputCoordsSnippet(outputInfo.shape, program.dispatchLayout);
    var sources = [
    ...
    if (!program.atomic) {
        sources.push(setOutputSnippet(outputInfo.shape, outputInfo.dtype, program.outputComponent));
    }
    program.variableNames.forEach(function (x, i) {
        sources.push("".concat(getCoordsFromIndexSnippet(inputInfo[i].shape, x)));
    });
    var inputSnippet = inputInfo
        .map(function (x, i) { return getInputSnippet(x, outputInfo.shape, program.variableComponents ? program.variableComponents[i] :
        program.outputComponent, program.dispatchLayout.x.length === outputInfo.shape.length); })
        .join('\n');
...
function makeShaderKey(program, inputsInfo, output) {
    ...
    var shapes = [];
    var types = [];
    inputsInfo.forEach(function (element) {
        shapes.push(element.shape);
        types.push(element.dtype);
    });
    shapes.push(output.shape);
    types.push(output.dtype);
    var broadcastDims = inputsInfo.map(function (d) { return tf.backend_util.getBroadcastDims(d.shape, output.shape); });
    var inputShapesEqualsOutShape = inputsInfo.map(function (d) { return tf.util.arraysEqual(d.shape, output.shape); }).join('_');
...
var WebGPUBackend = /** @class */ (function (_super) {
    ...
    function WebGPUBackend(device, adapterInfo) {
        ...
        _this.isGetShader = false;
        if (!isWebGPUSupported()) {
            throw new Error('WebGPU is not supported on this device');
        }
        _this.pipelineOrShaderCache = {};
        _this.device = device;
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        _b.trys.push([0, 2, , 3]);
                        return [4 /*yield*/, Promise.all(Object.values(this.pipelineOrShaderCache))];
                    case 1:
                        pipelines =
                            _b.sent();
                        return [3 /*break*/, 3];
                    ...
                    case 3:
                        Object.keys(this.pipelineOrShaderCache).map(function (key, i) {
                            _this.pipelineOrShaderCache[key] = pipelines[i];
                        });
                        return [2 /*return*/];
                }
            });
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        ...
                        this.ensureComputePassEnded();
                        this.currentCommandEncoder.copyBufferToBuffer(buffer, 0, stagingBuffer, 0, size);
                        this.submitQueue();
    ...
    WebGPUBackend.prototype.readSync = function (dataId) {
        ...
        stagingDeviceStorage
            ...
            var readDataGPUToCPU = function (width, height, offset) {
                ...
                _this.currentCommandEncoder.copyBufferToTexture({
                    buffer: buffer,
    ...
    WebGPUBackend.prototype.readToGPU = function (dataId) {
        ...
        this.ensureCommandEncoderReady();
        this.ensureComputePassEnded();
        this.currentCommandEncoder.copyBufferToBuffer(resource, 0, buffer, 0, size);
        this.submitQueue();
    ...
    WebGPUBackend.prototype.runWebGPUProgram = function (program, inputs, outputDtype, programDefinedUniform, output) {
        ...
        var inputsInfo = inputs.map(function (input, i) {
        ...
        program.shaderKey =
            makeShaderKey(program, inputsInfo, output);
        ...
        if (program.shaderKey in this.pipelineOrShaderCache) {
            ...
            program.pipeline =
                this.pipelineOrShaderCache[program.shaderKey];
        }
        else {
            var shader = makeShader(inputsInfo, output, program);
            if (this.isGetShader) {
                this.pipelineOrShaderCache[program.shaderKey] = shader;
                return output;
            }
            program.pipeline = compileProgram(this.device, program, shader, parallelCompilation);
     ...
     */
    // tslint:disable-next-line: no-any
    WebGPUBackend.prototype.getShader = function (inputSpecs) {
        if (inputSpecs == null) {
            throw new Error("Input spec is not provided!");
        }
        var savedFlag = tf__namespace.env().getBool('WEBGPU_CPU_FORWARD');
        ...
        tf__namespace.env().set('WEBGPU_CPU_FORWARD', savedFlag);
        this.isGetShader = false;
        var shaderTmp = Object.values(this.pipelineOrShaderCache);
        this.pipelineOrShaderCache = {};
        return shaderTmp;
    };
...
// tslint:disable-next-line: no-any
function callOp(inputSpec) {
    var name = inputSpec.name, inputs = inputSpec.inputs, args = inputSpec.args;
    var tensorArray = [];
    try {
        // tslint:disable-next-line: no-any
        inputs.forEach(function (input) {
            tensorArray.push(tf__namespace.zeros(input.shape, input.dtype));
        });
    }
    catch (_a) {
        throw new Error("Input spec ".concat(JSON.stringify(inputSpec), " is incomplete!"));
    }
    // tslint:disable-next-line: no-any
    var tfOpFunc = tf__namespace[name];
    if (typeof tfOpFunc === 'undefined') {
        throw new Error("Operator ".concat(name, " is not defined in ").concat(tf__namespace.engine().backendName, " backend"));
    }
    if (typeof args === 'undefined') {
        tfOpFunc.apply(void 0, __spreadArray([], __read(tensorArray), false)).dispose();
    }
    else {
        tfOpFunc.apply(void 0, __spreadArray(__spreadArray([], __read(tensorArray), false), __read(args), false)).dispose();
    }
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    return [4 /*yield*/, requestAdapter(gpuDescriptor)];
                case 1:
                    adapter = _a.sent();
                    deviceDescriptor = {};
...
(function (BinaryOpType) {
    ...
    BinaryOpType[BinaryOpType["GREATER"] = 7] = "GREATER";
    BinaryOpType[BinaryOpType["GREATER_EQUAL"] = 8] = "GREATER_EQUAL";
    BinaryOpType[BinaryOpType["INT_DIV"] = 9] = "INT_DIV";
...
function getBinaryOpString(type, useVec4) {
    ...
    do {
        ...
        return "\n      let isNaN = false;\n      let valueForNaN = uniforms.NAN;\n      {\n        ".concat(doOpSnippet, "\n        ").concat(useVec4 ? CHECK_NAN_SNIPPET_VEC4 : CHECK_NAN_SNIPPET, "\n        return resultTemp;\n      }\n    ");
    } while (false);
 ...
 * =============================================================================
 */
var floorDiv = binaryKernelFunc({ opType: BinaryOpType.INT_DIV,
